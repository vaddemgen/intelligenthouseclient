...
public final class Application {
    ...
    public static void main(String[] args)
        throws IOException, UnsupportedBusNumberException, PlatformAlreadyAssignedException, InterruptedException {
        ...
        while (true) {
            Bme280Value bme280Value = readBme280Value(device);

            // Output data to screen
            System.out.print(bme280Value);
    ...
    private static Bme280Value readBme280Value(I2CDevice device) throws IOException {
        ...
        long adc_p =
            (((long) (data[0] & 0xFF) * 65536) + ((long) (data[1] & 0xFF) * 256) + (long) (data[2]
                & 0xF0)) / 16;
        long adc_t =
            (((long) (data[3] & 0xFF) * 65536) + ((long) (data[4] & 0xFF) * 256) + (long) (data[5]
                & 0xF0)) / 16;
        ...
        var_H = (adc_h - (dig_H4 * 64.0 + dig_H5 / 16384.0 * var_H)) * (dig_H2 / 65536.0 * (1.0
            + dig_H6 / 67108864.0 * var_H * (1.0 + dig_H3 / 67108864.0 * var_H)));
        ...
        return new Bme280Value(cTemp, fTemp, pressure, humidity);
    }
}
